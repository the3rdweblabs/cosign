...
description: Vendored canonical x402 protocol specification v2 - schemes, payloads, verification, settlement, facilitator interface.
---
...
Individual schemes and their per-network bindings are specified under [`specs/schemes/`](./schemes/). For this deployment the only scheme in use - and the only one we document here - is our [`exact` on BOT Chain](./schemes/exact/scheme_exact_botchain.md).
...
> **Cosign note (BOT Chain):** this facilitator implements the `exact` scheme on
> BOT Chain's native currency using the chain's native EOA paymaster
> (BEP-414-style) for gas-free sponsorship, plus a mandatory self-pay fallback -
> not EIP-3009/Permit2 (BOT is native, not ERC-20). Default flow is
> `authorization` (verify → resource → settle → respond). See
> [scheme_exact_botchain.md](./schemes/exact/scheme_exact_botchain.md).
...
[`specs/extensions/bazaar.md`](./extensions/bazaar.md), since this endpoint is extension-specific behavior.
